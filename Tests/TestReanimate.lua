...
table.insert(Events, Character.ChildAdded:Connect(function(g)
	...
	if g:IsA("Tool") then pcall(function() g:Remove(); Player.Backpack[g.Name]:Destroy() end) end
end))
...
for i,v in pairs(CharDescendants) do
	if v:IsA("BasePart") then
		local awooga = Instance.new("BodyAngularVelocity")
		awooga.AngularVelocity = Vector3.new(math.huge,math.huge,math.huge)
		awooga.P = math.huge
		awooga.AngularVelocity = Vector3.new(0,0,0)
		local awooga2 = Instance.new("BodyVelocity")
		awooga2.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
		awooga2.P = math.huge;
		awooga.Parent = v
		awooga2.Parent = v
		table.insert(funi, awooga2)
...
task.spawn(function()
	task.wait(0.5)
	if PlayerRigType == "R15" then Character.HumanoidRootPart:Destroy();  Character.PrimaryPart = Character.Head; game.Players.LocalPlayer.ReplicationFocus = Character end
end)
Stuff.CreateSignal("RunService", "Heartbeat", function()
	Character.PrimaryPart = Character.LowerTorso
	local Vector = Dummy.HumanoidRootPart.CFrame.LookVector * 39
	local X,Z = Vector.X,Vector.Z
	Velocity = Vector3.new(X,math.random(0,10),Z)
	for i,v in ipairs(funi) do
	...
	pcall(function()
		if PlayerRigType == "R6" then
			...
			API.Align(Character["Head"], Dummy["Head"])

...
if getgenv().OGChar:FindFirstChild("Bullet") then
	...
	Thrust.Force = Vector3.new(260,260,260)
...
--                h:TakeDamage(Damage)
				if getgenv().OGChar:FindFirstChild("Bullet") then
					AttackPlayerWithFling(h.Parent)
				end
                else
				if getgenv().OGChar:FindFirstChild("Bullet") then
					AttackPlayerWithFling(h.Parent)
...
ReanimateAPI.Align(Character[Hat1].Handle, handle,Vector3.new(0,1.8,-1.8),Vector3.new(45,0,0))
...
Lite.Brightness = 10
Lite.Shadows = true
local MainTheme = it("Sound", Torso)
MainTheme.Looped = true
MainTheme.Volume = 0.3
if getgenv().FrostwareConfig.DisableAudio == false then
	ReanimateAPI.LoadMusicFromFiles(MainTheme,"FrostwareSongs/YellowBlade.mp3")
end
MainTheme:Play()
...
Neck.C0 = clerp(Neck.C0,necko *angles(math.rad(-10),math.rad(10),math.rad(-50)),.3)
...
Neck.C0 = clerp(Neck.C0,necko *angles(math.rad(5),math.rad(0),math.rad(50)),.3)
...
table.insert(getgenv().FrostwareConfig.TableOfEvents,mouse.KeyDown:connect(function(k)
        k=k:lower()
        if k=='q' then
...
Neck.C0=clerp(Neck.C0,necko*euler(-idleanim/2,0,0.4),.3)
...
Neck.C0 = clerp(Neck.C0, NeckCF * angles(math.rad(5 + 3 * math.cos((sine) / 23)), math.rad(0), math.rad(-2 * math.cos((sine) / 7))), 0.3)
